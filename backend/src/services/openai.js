...
console.log("[OPENAI_SDK_INITIALIZED]")
...
Adapte todo o tom, vocabulário e argumentos de venda ao perfil da categoria. Os textos devem parecer escritos por um corretor experiente que conhece profundamente o público deste tipo de imóvel.`
  ...
  // Monta array de mensagens com fotos (vision) se disponíveis
  const messages = []
  
  if (fotos && fotos.length > 0) {
    const content = []
    
    // Adiciona até 4 fotos
    fotos.slice(0, 4).forEach(foto => {
      content.push({
        type: 'image_url',
        image_url: {
          url: `data:${foto.tipo || 'image/jpeg'};base64,${foto.dados}`,
        },
      })
    })
  ...
  try {
    ...
    console.log("[OPENAI_RESPONSE_OK]")
